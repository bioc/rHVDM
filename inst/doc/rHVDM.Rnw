...
\item{Some prior biological knowledge about the transcription factor under review: at least three genes should be known to be targets of that transcription factor and presumed to be targets of that transcription factor \emph{only}.} These genes comprise the \emph{training set}.
...
\item{The measurement error for each expression value should be known. Note that this is the technical error, not the biological error. See below for more details about this.}
\end{itemize}
...
\begin{itemize}
\item{be contained in an \Robject{exprSet} object (\Rpackage{Biobase} package)}
...
\end{itemize}
In addition to expression values, estimates for the standard deviation of the measurement error \emph{have to be supplied} (in the \Robject{se.exprs} slot of the expression set). A ``quick and dirty" way to create some of those would be to use:
<<quickanddirtymeaserrors>>=
anothereset<-fiveGyMAS5
anothereset@se.exprs<-abs(anothereset@exprs*0.1+5.0)
@
This supposes the noise to be 10 percent of the signal intensity, plus an additive component. Recall that this is a ``quick and dirty" method. We recommend using a procedure similar to the one we implemented originally~\cite{HVDM,err1,err2,err3}. We remove the spurious expression set:
<<deletedirtyset>>=
rm(anothereset)
@
as \Robject{fiveGyMAS5} contains everything we need. The pheno data of the expression set should contain specific fields. To access this phenodata, use the \Rpackage{Biobase} command:
<<trypdatacommand>>=
pData(fiveGyMAS5)
@
...
<<badgene>>=
RgHVDMtnf10l<-fitgene(eset=fiveGyMAS5,gene="202688_at",tHVDM=tHVDMp53)
@
\begin{Sinput}
...
\begin{itemize}
\item{Apparently, PERL is required. We recommend ActivePerl \url{www.activestate.com/Products/ActivePerl}.  Note that cygwin perl cannot be used.}
\item{Rtools is required (\url{www.murdoch-sutherland.com/Rtools/tools.zip}).  You need to make sure that all these tools are in the path (see \url{http://www.computerhope.com/issues/ch000549.htm} on how to do this).}
